...
let mapData=null,profileData=null,projection='flat',currentTab='atlas',revision=0,loaded={},busy=0,refreshing=false,mapScale=null;
...
const palette=['#251626','#6b2940','#b64938','#e88448','#f5c77a','#fff0c5'];
const diverging=['#484268','#7875b1','#b8abd6','#eee4d5','#e89b67','#c85d39','#763325'];
const titles={guide:['WINDS.<br><em>MEET WAVES.</em>','Choose a question, compare two hypotheses, and understand what the physics tells you.'],legacy:['REVISIT.<br><em>THE MODES.</em>','Original eigenfunctions read from local archives, with explicit normalization diagnostics.'],atlas:['MARS.<br><em>IN MOTION.</em>','Climb through the atmosphere. Follow the seasons. See what the winds reveal.'],structure:['DEEPER.<br><em>INTO MARS.</em>','Local profiles and geometric sections: temperature, circulation and shear from the surface to 200 km.'],season:['ONE PLANET.<br><em>MANY SEASONS.</em>','Isolate seasonal changes at a fixed location, altitude and time convention.'],acoustic:['WIND.<br><em>MEETS SOUND.</em>','Directional diagnostics and sensitivity kernels: steps towards studying acoustic coupling.'],method:['CLEAR DATA.<br><em>OPEN METHODS.</em>','Sources, assumptions, conventions and limitations: each calculation should be reproducible.']};
...
async function refresh(overrides={}){
 ...
 try{
  ...
  if(token!==revision)return;state={...candidate};mapData=m;profileData=p;renderStats();drawMap();renderProfiles();
  $('provenance').textContent=JSON.stringify(m.provenance,null,2);
  refreshing=false;document.dispatchEvent(new CustomEvent('marswind:updated'));await ensureTab();
 }finally{if(token===revision){$('apply').disabled=false;refreshing=false;}}
}
...
function range(){if(mapScale)return mapScale;let a=mapData.fields[state.field].flat().filter(Number.isFinite);let min=Math.min(...a),max=Math.max(...a);const signed=['u','v','w'].includes(state.field)||min<0;if(signed){max=Math.max(Math.abs(min),Math.abs(max));min=-max;}else if(state.field!=='temperature')min=0;return {min,max,colors:signed?diverging:palette};}
...
function drawMap(){
 ...
 const W=canvas.clientWidth,H=canvas.clientHeight;if(!W||!H)return;canvas.width=W*dpr;canvas.height=H*dpr;ctx.scale(dpr,dpr);ctx.fillStyle='#100e14';ctx.fillRect(0,0,W,H);
 ...
 // Each displayed cell is one MCD query sample; no spatial smoothing.
 const cellColors=mapData.fields[state.field].map(row=>row.map(v=>Number.isFinite(v)?color((v-min)/(max-min||1),colors):null));
 if(projection==='flat'){
  const cw=geom.w/72,ch=geom.h/36;
  for(let j=0;j<36;j++)for(let i=0;i<72;i++){const rgb=cellColors[j][i];if(!rgb)continue;ctx.fillStyle=`rgb(${rgb.join(',')})`;ctx.fillRect(geom.x+i*cw,geom.y+(35-j)*ch,cw+.5,ch+.5);}
 }else{
  ...
  for(let y=0;y<H;y++)for(let x=0;x<W;x++){const ll=invert(x,y);if(!ll)continue;const q=valueAt(...ll),rgb=cellColors[q.j][q.i];if(!rgb)continue;const p=(y*W+x)*4;im.data[p]=rgb[0];im.data[p+1]=rgb[1];im.data[p+2]=rgb[2];im.data[p+3]=255;}
  sc.putImageData(im,0,0);ctx.drawImage(small,0,0);
 }
 ctx.lineWidth=.6;ctx.strokeStyle='#f7d5b628';
 ...
 if(projection==='flat'){ctx.font='8px monospace';ctx.fillStyle='#c1a18e';ctx.textAlign='center';for(let l=-180;l<=180;l+=60){const [x]=project(l,0);ctx.fillText(l+'°',x,H-33);}ctx.textAlign='right';for(let p=-60;p<=60;p+=30){const [,y]=project(0,p);ctx.fillText(p+'°',34,y+3);}}
 if($('vectors').checked){ctx.strokeStyle='#fff0d7a6';ctx.lineWidth=.8;for(let j=2;j<34;j+=3)for(let i=1;i<72;i+=3){const lat=mapData.latitude[j],lon=mapData.longitude[i],u=mapData.fields.u[j][i],v=mapData.fields.v[j][i];if(u==null||Math.hypot(u,v)<1)continue;const [x,y,visible]=project(lon,lat);if(!visible)continue;const epsilon=.05,sp=Math.hypot(u,v);const [xx,yy,vv]=project(lon+epsilon*u/sp/Math.cos(lat*Math.PI/180),lat+epsilon*v/sp);if(!vv)continue;const a=Math.atan2(yy-y,xx-x),len=8;ctx.beginPath();ctx.moveTo(x-Math.cos(a)*len/2,y-Math.sin(a)*len/2);const ex=x+Math.cos(a)*len/2,ey=y+Math.sin(a)*len/2;ctx.lineTo(ex,ey);ctx.moveTo(ex-3*Math.cos(a-.5),ey-3*Math.sin(a-.5));ctx.lineTo(ex,ey);ctx.lineTo(ex-3*Math.cos(a+.5),ey-3*Math.sin(a+.5));ctx.stroke();}}
...
function layout(extra={}){return {paper_bgcolor:'#1b171b',plot_bgcolor:'#1b171b',font:{family:'Arial, sans-serif',color:'#c8ada0',size:11},margin:{l:65,r:25,t:20,b:52},showlegend:true,legend:{orientation:'h',x:0,y:1.12,font:{size:10}},xaxis:{gridcolor:'#46313a',zerolinecolor:'#94674f',title:{font:{size:10}}},yaxis:{gridcolor:'#46313a',zerolinecolor:'#94674f',title:{text:'Altitude / areoid (km)',font:{size:10}}},hovermode:'closest',...extra};}
...
function renderProfiles(){if(!profileData)return;const f=profileData.fields,z=profileData.altitude_km;
 ...
 Plotly.react('miniProfile',[lineTrace(f.u,z,'u · east','#f2a06e'),lineTrace(f.v,z,'v · north','#b9a0dd')],layout({margin:{l:52,r:20,t:30,b:42},xaxis:{title:{text:'Wind (m/s)'},gridcolor:'#46313a'},yaxis:{title:{text:'Altitude (km)'},gridcolor:'#46313a'}}),config);
 const traces=[lineTrace(f.u.map((v,i)=>v-f.u_rms[i]),z,'u − RMS','#e9ccb8',{line:{width:0},showlegend:false}),lineTrace(f.u.map((v,i)=>v+f.u_rms[i]),z,'u ± RMS','#e9ccb8',{line:{width:0},fill:'tonextx',fillcolor:'#c1855430'}),lineTrace(f.u,z,'u · east','#ed9265'),lineTrace(f.v,z,'v · north','#b9a0dd'),lineTrace(f.temperature,z,'T (K)','#b09ecf',{xaxis:'x2',yaxis:'y2'}),lineTrace(f.shear,z,'Shear','#ecb8c8',{xaxis:'x3',yaxis:'y3'})];
 Plotly.react('profileChart',traces,layout({xaxis:{domain:[0,.43],title:{text:'Wind (m/s)'},gridcolor:'#46313a'},xaxis2:{domain:[.52,.73],title:{text:'Temperature (K)'},gridcolor:'#46313a'},xaxis3:{domain:[.82,1],title:{text:'Shear (m/s/km)'},gridcolor:'#46313a'},yaxis2:{anchor:'x2',matches:'y',showticklabels:false,gridcolor:'#46313a'},yaxis3:{anchor:'x3',matches:'y',showticklabels:false,gridcolor:'#46313a'}}),config);
 Plotly.react('acousticChart',[lineTrace(f.sound_speed,z,'c at rest','#b09ecf'),lineTrace(f.effective_speed,z,'c effective','#f2a06e'),lineTrace(f.along_wind,z,'Projected wind','#ddb18a')],layout({xaxis:{title:{text:'Speed (m/s)'},gridcolor:'#46313a'},shapes:[{type:'line',x0:0,x1:0,y0:0,y1:1,yref:'paper',line:{dash:'dot',color:'#b48565',width:1}}]}),config);
}
async function ensureTab(){
 if(!mapData||refreshing)return;const token=revision;
 if(currentTab==='legacy'&&!loaded.legacy){const a=await api('legacy');if(token!==revision)return;const traces=[];a.modes.forEach((m,i)=>{const c=palette[i%palette.length];traces.push(lineTrace(m.U.real.map(v=>v+i*2.5),m.altitude_km,'n = '+i,c,{showlegend:false}));traces.push(lineTrace(m.V.real.map(v=>v+i*2.5),m.altitude_km,'n = '+i,c,{showlegend:false,xaxis:'x2',yaxis:'y2'}));});const ticks=a.modes.map((m,i)=>i*2.5),names=a.modes.map(m=>'n'+m.n);Plotly.react('legacyChart',traces,layout({showlegend:false,xaxis:{domain:[0,.45],title:{text:'Radial component √ρ U'},tickvals:ticks,ticktext:names,gridcolor:'#46313a'},xaxis2:{domain:[.55,1],title:{text:'Horizontal component √ρ V'},tickvals:ticks,ticktext:names,gridcolor:'#46313a'},yaxis:{title:{text:'Archived-model altitude (km)'},gridcolor:'#46313a'},yaxis2:{anchor:'x2',matches:'y',showticklabels:false,gridcolor:'#46313a'}}),config);$('legacyTable').querySelector('tbody').innerHTML=a.modes.map(m=>`<tr><td>${m.n}</td><td>${m.max_sqrt_rho_U.toExponential(3)}</td><td>${m.max_sqrt_rho_V.toExponential(3)}</td><td>${fmt(m.normalization_integral_ratio,0)}</td><td>${fmt(m.atmospheric_inertia_fraction*100,2)} %</td><td>${fmt(m.lower_10km_fraction_of_atmosphere*100,3)} %</td></tr>`).join('');loaded.legacy=true;}
 ...
 if(currentTab==='season'&&!loaded.season){const s=await api('seasonal');if(token!==revision)return;Plotly.react('seasonChart',[lineTrace(s.Ls,s.fields.u,'u · east','#ed9265'),lineTrace(s.Ls,s.fields.v,'v · north','#b9a0dd'),lineTrace(s.Ls,s.fields.speed,'Horizontal speed','#b09ecf')],layout({xaxis:{title:{text:'Solar longitude Ls (°)'},dtick:45,range:[0,360],gridcolor:'#46313a'},yaxis:{title:{text:'Wind (m/s)'},gridcolor:'#46313a'}}),config);$('seasonLocation').textContent=`${fmt(state.lon,2)}° E · ${fmt(state.lat,2)}° N · ${state.altitude} km`;await compare();loaded.season=true;}
