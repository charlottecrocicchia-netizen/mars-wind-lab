...
function studyParameters(){const z_min=+$('studyBottom').value,z_max=+$('studyTop').value;return {lon:+$('studyLon').value,lat:+$('studyLat').value,ls:+$('studySeason').value,compare_ls:+$('studySeasonB').value,lt:+$('studyHour').value,azimuth:+$('studyDirection').value,z_min,z_max,period_s:+$('studyPeriod').value};}
function studyChanged(){
 ...
 $('studySeasonValue').textContent='Ls '+$('studySeason').value+'°';
 $('studyTopSlider').value=$('studyTop').value;
 $('studyTop').setCustomValidity(+$('studyTop').value-+$('studyBottom').value<5?'Choose a layer at least 5 km thick.':'');
}
...
function renderStudy(){
 ...
 const chartLayout=layout({font:{family:'Arial, sans-serif',color:'#c8ada0',size:12},margin:{l:58,r:18,t:65,b:55},legend:{orientation:'h',x:0,y:1.19,font:{size:11}},xaxis:{title:{text:'Effective sound speed (m/s)'},gridcolor:'#46313a'},yaxis:{title:{text:'Altitude / areoid (km)'},gridcolor:'#46313a',range:[c.altitude_min_km,c.altitude_max_km]}});
 ...
 if(studyQuestion==='direction'){
  ...
  traces=[trace(f.sound_speed,'At rest','#b9a7a2',{line:{color:'#b9a7a2',width:2,dash:'dot'}}),trace(f.effective_speed,direction+'ward','#f1a272'),trace(f.opposite_effective_speed,opposite+'ward','#c1a4e4')];
  chartLayout.shapes=[{type:'line',xref:'paper',x0:0,x1:1,y0:p.altitude_km,y1:p.altitude_km,line:{color:'#d89b6f70',dash:'dot',width:1}}];
  ...
  traces=[trace(f.season_total_delta,'Total change','#f8dfc5',{line:{color:'#f8dfc5',width:3}}),trace(f.season_wind_delta,'Wind','#f2a06e'),trace(f.season_thermodynamic_delta,'Thermodynamics','#c1a4e4')];
  chartLayout.xaxis.title.text='Change B − A (m/s)';
  chartLayout.shapes=[{type:'line',x0:0,x1:0,y0:0,y1:1,yref:'paper',line:{color:'#b9a7a2',dash:'dot',width:1}}];
  ...
  traces=[trace(f.wavelength_density_scale_ratio.map(v=>v>0?v:null),'λ₀ / Hρ','#f1a272')];
  chartLayout.showlegend=false;chartLayout.margin.t=30;
  chartLayout.xaxis={type:'log',title:{text:'λ₀ / Hρ · logarithmic scale'},gridcolor:'#46313a'};
  chartLayout.shapes=[{type:'line',x0:1,x1:1,y0:0,y1:1,yref:'paper',line:{color:'#f2a06e',dash:'dot',width:2}}];
  chartLayout.annotations=[{x:0,xref:'x',y:1,yref:'paper',text:'λ₀ = Hρ',showarrow:false,xanchor:'left',yanchor:'bottom',font:{size:11,color:'#f3ae81'}}];
...
$('studyTopSlider').addEventListener('input',()=>{$('studyTop').value=$('studyTopSlider').value;studyChanged();});
